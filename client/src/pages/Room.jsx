  ...
  useEffect(() => {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        // PeerJS error / disconnect handling
        peer.on('error', (err) => {
        ...
        // Helper: attach stream/close/error listeners to a call
        const connectCall = (call, peerId, name) => {
          call.on('stream', (remoteStream) => {
            console.log('[PeerJS] Got remote stream from', peerId);
            setPeers((prev) => ({
              ...prev,
          ...
          call.on('error', (err) => {
            console.error('[PeerJS] Call error with', peerId, err);
          });
        };
        ...
        peer.on('call', (call) => {
          ...
          connectCall(call, call.peer, call.metadata?.userName || 'Participant');
          call.answer(stream);
        });

        // When a new user connects, call them (with retry)
        socket.on('user-connected', (peerId, userName) => {
          console.log('[PeerJS] user-connected, calling', peerId);
          const tryCall = (attempt = 1) => {
            const call = peer.call(peerId, stream, {
              metadata: { userName: user?.name },
            });
            if (!call) {
              if (attempt < 4) {
                console.warn(`[PeerJS] peer.call returned null, retry ${attempt}…`);
                setTimeout(() => tryCall(attempt + 1), 2000);
              }
              return;
            }
            connectCall(call, peerId, userName);
            peersRef.current[peerId] = call;
          };
          // Delay to let the remote peer fully register
          setTimeout(() => tryCall(), 1500);
        });
